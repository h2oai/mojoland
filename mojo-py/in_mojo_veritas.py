...
if __name__ == "__main__":
    ...
    parser.add_argument("--filter", help="Taste only recipes matching given filter")
    ...
    if args.filter:
        filter = re.compile(args.filter, flags=re.I)
        recipes = [rcp for rcp in recipes if filter.search(rcp.__name__)]
    if not recipes:
        raise ValueError("No recipes to run;\nValid recipes are: %s" %
                         (", ".join(rcp.__name__[:-6] for rcp in mojoland.list_recipes())))
